...
def find_similar_category(category_name, is_main_category=True, accept_new=True):
    ...
    Eğer yeterince benzer bir kategori bulunamazsa, yeni bir kategori oluşturulması gerektiğini belirtir.
    ...
    try:
        ...
        # Benzerlik düşükse ve yeni kategorilere izin veriliyorsa, yeni kategori öner
        if accept_new:
            print(f"Benzer kategori bulunamadı veya benzerlik düşük (%{highest_score*100:.1f}). Yeni {category_type.lower()} önerilecek: '{category_name}'")
            return True, category_name
        else:
            # Yeni kategorilere izin verilmiyorsa, en benzer kategoriyi kullan veya boş döndür
            if most_similar:
                print(f"Yeni kategorilere izin verilmiyor. En yakın kategori kullanılacak (%{highest_score*100:.1f}): '{most_similar}'")
                return False, most_similar
            else:
                print(f"Benzer kategori bulunamadı ve yeni kategorilere izin verilmiyor. Boş kategori döndürülüyor.")
                return False, ""
        ...
        return accept_new, category_name
...
def match_categories_and_tags(json_result, existing_categories=None, existing_tags=None):
    ...
    for category in json_result.get('categories', []):
        ...
        # Kategori ilgili mi? Puanlama fonksiyonu kullanarak hesapla
        relevance_score = calculate_category_relevance(matched_category, json_result)
        print(f"'{matched_main} > {matched_sub}' kategorisi için ilgililik puanı: {relevance_score:.2f}")
        
        # Eğer ilgililik puanı yeterince yüksekse (0.60'dan büyükse), bu kategoriyi ekle
        if relevance_score >= 0.60:  # İlgililik eşiğini %60'a ayarladık
            matched_categories.append(matched_category)
            print(f"Yeterli ilgililik puanıyla kategori eklendi: '{matched_main} > {matched_sub}'")
        else:
            print(f"Düşük ilgililik puanı nedeniyle kategori eklenmedi: '{matched_main} > {matched_sub}'")
    
    # En çok 3 kategori ekle
    max_categories = 3
    
    # En çok ilgili kategorileri seç
    if len(matched_categories) > max_categories:
        print(f"Çok fazla kategori var ({len(matched_categories)}), sadece en ilgili {max_categories} tanesi seçilecek")
        
        # İlgililik puanına göre kategorileri sırala
        relevance_scores = []
        for category in matched_categories:
            score = calculate_category_relevance(category, json_result)
            relevance_scores.append((category, score))
        
        # Puanlara göre sırala (büyükten küçüğe)
        relevance_scores.sort(key=lambda x: x[1], reverse=True)
        
        # En ilgili olanları seç
        matched_categories = [item[0] for item in relevance_scores[:max_categories]]
    
    # Eğer hiç kategori eşleşmediyse, içerikten otomatik kategori oluştur
    if not matched_categories:
        print("Hiç kategori eşleşmedi, otomatik kategori oluşturuluyor...")
...
def calculate_category_relevance(category, json_result):
    ...
    """
    print(f"\nKategori ilgililik puanı hesaplanıyor: {category}")
    ...
    # Başlık ve açıklamada ana kategori geçiyor mu?
    if main_category in title or main_category in description:
        score += 0.4
        print(f"Ana kategori '{main_category}' başlık veya açıklamada bulundu, skor: {score}")
    
    # Başlık ve açıklamada alt kategori geçiyor mu?
    if subcategory in title or subcategory in description:
        score += 0.4
        print(f"Alt kategori '{subcategory}' başlık veya açıklamada bulundu, skor: {score}")
    
    # Etiketlerde ana kategori veya alt kategori geçiyor mu?
    tags = json_result.get('tags', [])
    if tags:
        for tag in tags:
            tag = tag.lower()
            if main_category in tag or subcategory in tag:
                score += 0.2
                print(f"Kategori etiketlerde bulundu, skor: {score}")
                break
    
    print(f"Final ilgililik puanı: {score}")
    return score
